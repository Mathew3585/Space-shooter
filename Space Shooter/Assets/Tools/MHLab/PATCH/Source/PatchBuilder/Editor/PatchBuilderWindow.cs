...
using CompressionType = MHLab.PATCH.Compression.CompressionType;
...
public class PatchBuilderWindow : EditorWindow
{
    ...
    private void Deploy()
    {
		...
		{Debug.Log(SettingsManager.DEPLOY_PATH); Debug.Log(SettingsManager.BUILDS_PATH); Debug.Log(SettingsManager.APP_PATH); 
	        ...
	        EditorUserBuildSettings.SwitchActiveBuildTarget(BuildPipeline.GetBuildTargetGroup(cachedTarget), cachedTarget);
	        EditorUtility.ClearProgressBar();
		}
		catch(Exception e)
		{
